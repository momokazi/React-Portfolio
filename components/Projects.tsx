          ...
          {featuredProjects.map((project, index) => (
            ...
            >
              ...
              <div className="w-full md:w-24 bg-slate-900 flex md:flex-col items-center justify-between p-6 border-b md:border-b-0 md:border-r border-slate-800 shrink-0">
                 <div 
                   className="w-12 h-12 rounded-xl flex items-center justify-center bg-white p-1 shadow-lg"
                 >
              ...
              <div className="p-8 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex gap-2">
                    {project.githubUrl && (
                      <a href={project.githubUrl} target="_blank" className="p-2 bg-slate-900 text-slate-400 hover:text-white rounded-lg transition-colors border border-slate-800 hover:border-slate-600">
                        <Github size={18} />
                      </a>
                    )}
                  </div>
                ...
                <div className="pt-6 border-t border-slate-900 mt-auto">
                   <div className="flex flex-wrap gap-2">
                    {project.techStack.map((tech) => (
                      <span 
                        key={tech} 
                        className="text-xs font-medium px-3 py-1.5 bg-slate-900 text-slate-300 rounded-lg border border-slate-800 group-hover:border-slate-700 transition-colors"
                      >
                        {tech}
                      </span>
                    ))}
